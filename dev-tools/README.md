...
- Python 3.11 사용 가능
- `dev-tools\Runtimes\Shared` 심볼릭 링크가 정상이어야 하거나, 원본 허브 저장소 `C:\Users\HG\Documents\Contexthub\Runtimes\Shared`가 존재해야 함
...
- `sync-shared-runtime.ps1`: `dev-tools\Runtimes\Shared`를 허브 저장소 `Runtimes\Shared`로 미러링
...
- 기본 개발 기준 경로는 `dev-tools\Runtimes\Shared`다. 없으면 허브 저장소의 `Runtimes\Shared`를 사용한다.
- `dev-tools\runtime_Old`는 레거시 수동 미러다. 새 작업 기준 경로로 사용하지 않는다.
