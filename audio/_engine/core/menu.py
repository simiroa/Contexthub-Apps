...
def build_handler_map():
    ...
    return {
        ...
        # === Audio ===
        "audio_convert": lambda p, s=None: gui_popen([PYTHONW_EXE, str(app_root / "audio_convert" / "main.py"), *([str(i) for i in s] if s else [str(p)])]),
...
def main():
    try:
        ...
        logger.error(f"Context menu failure: {e}")
