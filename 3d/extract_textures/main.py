...
def _show_confirm(targets: list[Path]) -> bool:
    ...
    request = ConfirmRequest(
        ...
        subtitle="Confirm the selected 3D files, then extract embedded textures through Blender.",
        item_count=len(targets),
        item_label="files",
        output_rule="Creates or reuses a textures folder in the source file location.",
...
def main() -> int:
    ...
    from features.mesh.mesh_console import run_extract_textures_console

    return run_extract_textures_console(targets)
