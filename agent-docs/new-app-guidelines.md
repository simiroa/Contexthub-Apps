...
- GUI 앱이면 먼저 `agent-docs/gui-runtime-contract.md`, `agent-docs/gui-runtime-status.md`를 보고 템플릿 버킷을 정한다.
  - **Market Golden Recipes (참조용 앱)**:
    - **`mini`**: `extract_textures`, `extract_bgm`, `cad_to_obj` 참조.
    - **`compact`**: `auto_lod`, `doc_convert`, `simple_normal_roughness` (구현 참조: `auto_lod_qt_window.py`).
    - **`full`**: `audio_toolbox`, `doc_scan`, `merge_to_exr`, `image_compare` (구현 참조: `upscale_qt_app.py`, `bg_removal_qt_app.py`).
    - **`special`**: `versus_up`, `qwen3_tts`, `ai_text_lab` 참조.
- **주의**: 새 앱을 구축할 때 가상의 컴포넌트(phantom Cards: `ExportRunPanel`, `PresetParameterPanel`, `ParameterControlsPanel`, `QueueManagerPanel`, `ResultInspectorPanel`, `VideoPreviewCard`)를 복제하거나 사용하도록 지시하지 않는다. 오직 실제로 존재하는 API surface만 사용해야 한다.
- 새 Qt 앱은 가능하면 기존 앱 복제보다 `qt-app-builder-contexthub`의 템플릿 자산과 위 골든 앱들을 시작점으로 삼는다.
