...
def resolve_python():
    ...
    if settings.get("AI_ENV_MODE", "prefer_conda") != "disabled":
        ...
        if conda_exe:
            ...
            try:
                logger.warning(
                    "AI Conda Recommended: AI tools prefer a Conda environment. "
...
def _open_manager():
    ...
    try:
        ...
        logger.error(f"{t('common.error')}: {t('menu.open_manager_failed').format(error=e)}")
...
def build_handler_map():
    ...
    return {
        ...
        "esrgan_upscale": _lazy("features.ai.upscale_qt_app", "start_app"),
        "rmbg_background": _lazy("features.ai.bg_removal_qt_app", "start_app"),
