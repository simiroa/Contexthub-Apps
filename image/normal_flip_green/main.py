...
def _show_confirm(targets: list[Path]) -> bool:
    ...
    request = ConfirmRequest(
        app_root=APP_ROOT,
        title="Normal Flip Green",
        subtitle="Confirm the selected images, then continue in a console progress flow.",
        item_count=len(targets),
        item_label="images",
        output_rule="Creates *_flipped files next to each source image.",
        confirm_label="Flip",
    )
...
def main() -> int:
    ...
    return run_normal_flip_green_console(targets)
