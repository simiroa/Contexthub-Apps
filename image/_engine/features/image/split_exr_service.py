    ...
    def run_automation_split(self, 
        ...
        def _task():
            ...
            for i, f_path in enumerate(files):
                if self._cancel_flag: break
                try:
                    on_progress(i / total, f"Analyzing: {f_path.name}")
                    info, layers = self.analyze_file(f_path)
                    
                    # Generate configs for ALL layers
                    configs = []
                    for l in layers:
                        configs.append({
                            'name': l['name'],
                    ...
                    if f_path.suffix.lower() == ".exr":
                        self._split_exr(f_path, configs, out_dir, format_ext)
                    else:
                        self._split_generic(f_path, configs, out_dir, format_ext)
